...
snd_dir = path.join(path.dirname(__file__), "wav")
WIDTH = 480
HEIGHT = 600
FPS = 60
POWERUP_TIME = 5000
...
def draw_shield_bar(suf, x, y, pct):
    if pct < 0:
        pct = 0
    BAR_LENGTH = 100
    BAR_HEIGHT = 10
    ...
    pygame.draw.rect(suf, GREEN, fill_rect)
    pygame.draw.rect(suf, WHITE, outline_rect, 2)
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        # unhide if hidden
        if self.hidden and pygame.time.get_ticks() - self.hiden_timer > 1000:
            self.hidden = False
            self.rect.centerx = WIDTH / 2
            self.rect.bottom = HEIGHT - 10
...
class Mob(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image_1 = random.choice(meteor_images)
    ...
    def rotate(self):
        ...
        if now  - self.last_update > 50:
            ...
            new_image = pygame.transform.rotate(self.image_1, self.rot)
...
laser_img = pygame.image.load(path.join(img_dir, "laserBlue13.png")).convert()
meteor_images = []
...
for anim in range(9):
    filename = 'regularExplosion0{}.png'.format(anim)
    ...
    filename2 = 'sonicExplosion0{}.png'.format(anim)
...
powerup_images['gun'] = pygame.image.load(path.join(img_dir, 'powerupBlue_bolt.png')).convert()

# load game sounds 
shoot_sound = pygame.mixer.Sound(path.join(snd_dir, "Laser_Shoot.wav"))
exp_sound =[]
exp_list = ['Explosion.wav', 'Explosion5.wav']

for i in exp_list:
    exp_sound.append(pygame.mixer.Sound(path.join(snd_dir, i )))
die_snd = pygame.mixer.Sound(path.join(snd_dir, 'rumble1.ogg'))
...
while running:
    ...
    for hit in hits:
        ...
        if random.random() > 0.9:
            ...
            powerups.add(pow)
        newmob()
    # check if the player hit the power 
    ...
    for hit in hits:
        ...
        # if the died and the explosion has finished 
